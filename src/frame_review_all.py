...
class ReviewFrameAll(ctk.CTkFrame):
    ...
    def update_review_interval_correct(self):
        self.correct += 1

        cardToUpdate = self.current_card.id
        currentInterval = min(8, max(1, self.current_card.interval * 2))
    ...
    def update_review_interval_wrong(self):
        self.mistakes += 1
        self.mistakes_list.append(self.current_card)
